...
// import { WishlistButton,handleWishlistToggle } from './WishlistButton';
...
const ProductDetail = () => {
  ...
  const findLowestPricedVariant = (variants) => {
    ...
    return activeVariantsWithStock.reduce((lowest, current) => (current.price < lowest.price ? current : lowest))
  }
  ...
  const getAvailableColors = (size) => {
    const activeVariants = getActiveVariants()
    return [...new Set(activeVariants.filter((v) => v.size.name === size && v.stock_quantity > 0).map((v) => v.color))]
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="relative w-full max-w-[500px] mx-auto">
          ...
          <div className="relative mt-4">
            ...
            <div ref={scrollRef} className="flex space-x-2 overflow-x-hidden relative mx-8 scroll-smooth">
        ...
        <div className="relative">
          ...
          <div className="flex items-center mb-4">
            ...
            <span className="ml-2 inline-block bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full font-semibold">
          ...
          {selectedVariant && (
            <div className="flex items-baseline gap-2 mb-4">
              ...
              {product.discount_percentage > 0 && (
                <>
                  <div className="text-lg text-gray-500 line-through">₹{selectedVariant.price.toLocaleString()}</div>
                  <div className="text-sm text-red-500">({product.discount_percentage}% off)</div>
                </>
              )}
            </div>
          )}
          ...
          <div className="flex space-x-4">
            ...
            <WishlistButton variant={selectedVariant} onToggle={handleWishlistToggle} isInWishlist={isInWishlist} />
          </div>
        </div>
      </div>

      {/* Review Component */}
      {product && <ReviewComponent productId={product.id} />}
      ...
      <div className="mt-16">
        ...
        <div className="flex overflow-x-auto space-x-4 py-4 scrollbar-hide">
          {similarProducts.map((similar) => (
            <Link key={similar.id} to={`/user/product/${similar.id}`} className="group flex-shrink-0 w-48">
              <div className="border rounded-lg p-4 transition-transform hover:scale-105 shadow-lg">
                ...
                />
                <h3 className="font-medium text-gray-800 mb-2 truncate">{similar.name}</h3>
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-green-600 font-bold">
                      ₹
                      {(
                        Math.min(...similar.variants.map((v) => v.price)) *
                        (1 - (similar.discount_percentage || 0) / 100)
                      ).toLocaleString()}
                    </span>
                    {similar.discount_percentage > 0 && (
                      <div className="flex gap-2 items-center">
                        ...
                        <span className="text-xs text-red-500">({similar.discount_percentage}% off)</span>
                      </div>
                    )}
                  </div>
                  <div className="flex items-center">
                    {[...Array(5)].map((_, index) => (
                      <svg
                        key={index}
                        className={`w-4 h-4 ${
                          index < Math.round(similar.average_rating) ? "text-yellow-400" : "text-gray-300"
                        }`}
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
